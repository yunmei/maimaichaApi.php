...
class Api_App
{
	 ...
	 * 获取iphone5的加长图片
	 */
	public function getImageAdd($args,$format)
	{
		$criteria = new CDbCriteria();
		$criteria->addColumnCondition(array('state'=>1));
		$rowset = AddImage::model()->findAll($criteria);
		if($rowset[0]->id)
		{
			$array = array(array('imageUrl'=>$rowset[0]['image_url']));
		}
		$data = array('errorCode'=>ApiError::SUCCESS,'errorMessage'=>'','result'=>$array);
		return $data;
	}
	 ...
	 *version1.0
	 */
	public function getAppChangeList($args,$format)
	{
		$num = intval($_GET['num']);
		$num = $num ? $num : 8;
		$criteria = new CDbCriteria();
		...
		$criteria->order = 'usenum desc';
		$criteria->limit = $num;
		$rowset = Appchange::model()->findAll($criteria);
		$attributes = array('id','icon','name');
		foreach($rowset as $n)
		{
			$array[] = ApiBase::object2array($n,$attributes);
	 ...
	 */
	 ...
	 public function getAppChangeContent($args,$format)
	{
		$id = intval($_GET['id']);
		$criteria = new CDbCriteria();
	...
	public function getAppContentForAndroid($args, $format)
	
	{
		...
		$attributes = array('id', 'name', 'android_pack', 'android_class', 'android_url', 'html');
